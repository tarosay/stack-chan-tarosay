...
int lastStatus = 0;
unsigned long previousMillis = 0;     // 最後に処理した時間
const unsigned long interval = 5000;  // 待機時間（ミリ秒）
bool isDemo = false;
...
void loop() {
  ...
  int FileUploaded = webAPI.getFileUploaded();
  if (FileUploaded == 1) {
    // 拡張子チェック
    ...
    servo.moveXY(system_config.getServoInfo(AXIS_X)->start_degree, system_config.getServoInfo(AXIS_Y)->start_degree, 1000);
  } else if (FileUploaded == 0) {
    // 左右に向きを変えます
    // if (millis() - previousMillis >= interval) {
    //   int staX = system_config.getServoInfo(AXIS_X)->start_degree;
    //   int x = staX;
    //   if (lastStatus == 0) {
    //     //x = random(staX - 6, staX - 3);
    //     x = staX - 10;
    //     lastStatus = 1;
    //   } else if (lastStatus == 1) {
    //     //x = random(staX + 3, staX + 6);
    //     x = staX + 10;
    //     lastStatus = 0;
    //   }
    //   servo.moveXY(x, system_config.getServoInfo(AXIS_Y)->start_degree, 500);
    //   previousMillis = millis();  // 処理を実行した時間を記録
    // }
  } else if (FileUploaded == 100) {
    FaceUp();
    speech->playSound("/upload.wav", webAPI.getVolume());
    webAPI.resetFileUploadedType();
    servo.moveXY(system_config.getServoInfo(AXIS_X)->start_degree, system_config.getServoInfo(AXIS_Y)->start_degree, 1000);
  ...
  if (isDemo == true) {
    ...
    while (isDemo) {
      //正面を向く
      servo.moveXY(system_config.getServoInfo(AXIS_X)->start_degree, system_config.getServoInfo(AXIS_Y)->start_degree, 1000);
      speech->playSound("/wav/utaimasu.wav", webAPI.getVolume());

      //歌う歌をランダムに選曲する
      int num = random(0, musicListCount);
      String wavFilenameStr = "/wav/" + String(musicList[num].filename);
      Singing(wavFilenameStr, musicList[num].intoro, musicList[num].rhythm);
      delay(1000);

      faceCnt++;
      if (isDemo & faceCnt > 2) {
        int ongen = random(1, 5);
        FaceChangeSpeek(ongen);
        faceCnt = 0;
      }
    }
  ...
  if (webAPI.getIsSing()) {
    //歌う
    Singing(String(webAPI.getWavFilename()), webAPI.getIntoroTime(), webAPI.getRhythmTime());
...
//歌う
//**************************
void Singing(String wavFilenameStr, int intoro, int rhythm) {
  ...
  //前奏中、頭を上下する
  //unsigned long endMillis = millis() + webAPI.getIntoroTime() * 1000;
  unsigned long endMillis = millis() + intoro * 1000;
  x = system_config.getServoInfo(AXIS_X)->start_degree;
  //int rhythm = webAPI.getRhythmTime();
  while (millis() < endMillis) {
    //頭を上げる
    y = system_config.getServoInfo(AXIS_Y)->start_degree - 15;
    servo.moveXY(x, y, rhythm);
    //頭を下げる
    y = system_config.getServoInfo(AXIS_Y)->start_degree;
    servo.moveXY(x, y, rhythm);
  }
  ...
  while (wavPlayer.isPlaying()) {
    ...
    y = random(system_config.getServoInfo(AXIS_Y)->start_degree - 26, system_config.getServoInfo(AXIS_Y)->start_degree);
...
void FaceChangeSpeek(int num) {
  ...
  if (num == 1) {
    ...
    M5.Lcd.pushImage(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, yotsugis);
    wavPlayer.play(yotsugi1s, sizeof(yotsugi1s));
  }
