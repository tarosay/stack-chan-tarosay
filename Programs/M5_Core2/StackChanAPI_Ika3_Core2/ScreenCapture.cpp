...
bool ScreenCapture::load(const char* datpath) {
  File file = SD.open(datpath, FILE_READ);
  if (!file) {
...
void ScreenCapture::loadbin(const uint16_t* name) {
  uint16_t lineBuffer[SCREEN_WIDTH];
  for (int y = 0; y < SCREEN_HEIGHT; y++) {
    for (int x = 0; x < SCREEN_WIDTH; x++) {
      lineBuffer[x] = name[y * SCREEN_WIDTH + x];
    }
    M5.Lcd.pushImage(0, y, SCREEN_WIDTH, 1, lineBuffer);
  }
}