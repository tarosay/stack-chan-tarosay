...
const unsigned long interval = 2000;  // 待機時間（ミリ秒）
...
void loop() {
  ...
  if (FileUploaded == 1) {
    ...
    //   int x = staX;
    //   if (lastStatus == 0) {
    //     x = random(staX - 10, staX - 4);
    //     lastStatus = 1;
    //   } else if (lastStatus == 1) {
    //     x = random(staX + 4, staX + 10);
    //     lastStatus = 0;
    //   }
    //   servo.moveXY(x, system_config.getServoInfo(AXIS_Y)->start_degree, 200);
  ...
  if (webAPI.getIsSing()) {
    ...
    //10秒待つ 前奏のイメージ
    delay(webAPI.getIntoroTime() * 1000);
    //口パクパク開始
    while (wavPlayer.isPlaying()) {
