  ...
  server.on("/move", HTTP_GET, [this]() {
    ...
    for (int i = 0; i < server.args(); i++) {
      if (server.argName(i) == "m") {
        String arg = server.arg(i);  // "100,200,500" など
        byte x = 0, y = 0;
        short delay = 0;
        int firstComma = arg.indexOf(',');
        int secondComma = arg.indexOf(',', firstComma + 1);
        if (firstComma > 0 && secondComma > firstComma) {
          moveX[j] = arg.substring(0, firstComma).toInt();
          moveY[j] = arg.substring(firstComma + 1, secondComma).toInt();
          moveTimes[j] = arg.substring(secondComma + 1).toInt();
          isMoves = true;
    ...
    server.send(200, "text/plain", "move set ok");
  });
