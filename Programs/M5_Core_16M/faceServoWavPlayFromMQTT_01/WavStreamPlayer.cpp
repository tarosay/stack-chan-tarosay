...
bool WavStreamPlayer::beginAsync(uint32_t stackBytes, UBaseType_t prio, BaseType_t core, uint32_t periodMs) {
  ...
  // ★静的確保：heapを一切消費しない
  // 互換のため stackBytes が大きすぎたら失敗扱いにする（黙って切り詰めない）
  if (stackBytes > kSvcStackBytes) {
    lock_();
    taskRun_ = false;
    task_ = nullptr;
    unlock_();
  ...
  TaskHandle_t h = xTaskCreateStaticPinnedToCore(
    taskThunk_,
    "WavStreamSvc",
    (uint32_t)(kSvcStackBytes / sizeof(StackType_t)),
    this,
    prio,
    taskStack_,
    &taskTCB_,
...
bool WavStreamPlayer::streamBegin(uint32_t sampleRate, uint16_t channels,
  ...
  // ★先にFIFOを確保（28KBの連続領域を先取りして断片化負けを避ける）
  ensureStreamFifo_();
  if (!fifo_) {
    M5_LOGE("[STREAM] fifo alloc failed bytes=%u", (unsigned)fifoBytes_);
    streamMode_ = false;
    stopUnlocked_();  // lock中なので unlocked版を使用
    unlock_();        // ★unlock漏れ修正
    return false;
  }
  fifoHead_ = fifoTail_ = fifoUsed_ = 0;
...
bool WavStreamPlayer::preallocStream(size_t fifoBytes, size_t playChunkBytes) {
  ensureInternalMutex_();
  lock_();

  // FIFO
  fifoBytes_ = fifoBytes;
  ensureStreamFifo_();
  if (!fifo_) {
    unlock_();
    return false;
  ...
  // 再生チャンク用バッファ
  if (playChunkBytes < 256) playChunkBytes = 256;
  if (!buf0_ || bufCap0_ < playChunkBytes) {
    buf0_ = (uint8_t*)realloc(buf0_, playChunkBytes);
    bufCap0_ = buf0_ ? playChunkBytes : 0;
  }

  bool ok = (buf0_ != nullptr);
  unlock_();
  return ok;
}
