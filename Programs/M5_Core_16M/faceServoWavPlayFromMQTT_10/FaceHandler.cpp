...
bool FaceHandler::handle(const char* topic, uint8_t* payload, unsigned int len) {
  ...
  // face/mouth/size   payload: "size" (float 0..1)
  if (strcmp(a, "mouth") == 0) {
    if (strcmp(b, "size") == 0) {
      float size = 0.0f;
      uint32_t dummyMs = 0;
      // payload が "0.65" のように size だけでも parseSizeMs_ は読める（msは未指定ならデフォルト）
      if (parseSizeMs_(payload, len, size, dummyMs)) {
        fa_.postMouthSize(size);
      }
      return true;
    }
    return true;
