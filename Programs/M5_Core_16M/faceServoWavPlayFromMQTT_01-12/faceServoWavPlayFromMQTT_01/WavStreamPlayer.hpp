...
public:
  static constexpr uint32_t kSvcStackBytes = 2048;  // 必要なら 3072/4096 に
  ...
  bool beginAsync(uint32_t stackBytes = 2048, UBaseType_t prio = 2, BaseType_t core = 0, uint32_t periodMs = 1);
  void endAsync();
  ...
  void streamEnd();
  void streamClear();
  // ===== PCM16ストリーム用バッファを先に確保しておく =====
  bool preallocStream(size_t fifoBytes, size_t playChunkBytes);
...
private:
  ...
  StaticTask_t taskTCB_{};                                         // ★追加
  StackType_t taskStack_[kSvcStackBytes / sizeof(StackType_t)]{};  // ★追加
