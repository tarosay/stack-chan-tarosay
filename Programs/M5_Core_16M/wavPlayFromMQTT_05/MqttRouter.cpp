...
void MqttRouter::onMessage_(char* topic, uint8_t* payload, unsigned int length) {
  ...
  if (async_) {
    enqueue_(topic, payload, length);  // ★callback内はコピーしてキューへ
    return;
  }
...
void MqttRouter::enableAsyncDispatch(size_t queueDepth) {
  ...
  q_ = xQueueCreate(queueDepth, sizeof(RxItem*));
}
...
bool MqttRouter::dispatchOne(TickType_t waitTicks) {
  ...
  RxItem* m = nullptr;
  if (xQueueReceive(q_, &m, waitTicks) != pdTRUE) return false;
  if (!m) return false;

  // ここで「従来通り」filterマッチしたハンドラへ配る
  for (auto& s : subs_) {
